...
grpc_server* server_create(grpc_completion_queue* cq, char* server_addr) {
  ...
  grpc_server_credentials* server_creds =
      grpc_ssl_server_credentials_create_ex(
          ...
          GRPC_SSL_REQUEST_CLIENT_CERTIFICATE_AND_VERIFY, NULL);
...
grpc_channel* client_create(char* server_addr, grpc_ssl_session_cache* cache) {
  ...
  grpc_arg args[] = {
      ...
      grpc_channel_arg_pointer_create(
          const_cast<char*>(GRPC_SSL_SESSION_CACHE_ARG),
          cache, grpc_ssl_session_cache_arg_vtable()),
  };
...
void do_round_trip(
    grpc_completion_queue* cq, grpc_server* server, char* server_addr,
...
int main(int argc, char** argv) {
  ...
  roots_file =
      gpr_tmpfile("chttp2_ssl_session_reuse_test", &roots_filename);
